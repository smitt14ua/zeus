...
| `allowed_scopes` | string[] | Optional. Which command scopes this agent accepts. Absent or empty = all scopes except opt-in ones (currently none — `system` is included). See [Scope System](#scope-system). |
...
**Scope:** `system`. **Currently included in `all` and the default** (temporary, v0.7.1);
it will become opt-in again in a future release, requiring `--allow all,system`.
To exclude it now, start the agent with an explicit list, e.g. `--allow view,control,manage,update`.
...
| `update` | `update` |
| `system` | `system.reboot` — currently included in `all` and the default; will become opt-in (must be named explicitly) in a future release |
| `all` | All scopes except opt-in ones — currently all scopes (default when `--allow` is omitted) |
...
# Everything except opt-in scopes (same as default; currently includes system)
zeus agent --url wss://panel.example.com/ws/agent --token abc123 --allow all

# Full access including host reboot (future-proof once system becomes opt-in)
...
those in opt-in scopes (currently none; `system` becomes opt-in in a future release).
...
- `allowed_scopes` being absent in JSON is identical to `[]` — both mean
  "allow all except opt-in scopes" (currently none). Check with `len(scopes) == 0`
